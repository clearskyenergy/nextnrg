...
function ivUnderwrite(ctx){
  ...
  var depValue=0;
  if(dep.on){ var di; for(di=0;di<dep.shield.length;di++){ depValue += dep.shield[di]/Math.pow(1+disc,di+1); } }
  ...
  var netCapex = monetizeDepr ? (netCapexPreDep - depValue) : netCapexPreDep;
  if(netCapex<0) netCapex=0;

  /* --- capital stack --- */
  var cap=ivCapitalStack(netCapex,y1ebitda,life);
  var equityIn = cap.levered? cap.equity : netCapex;
  ...
  for(y=1;y<=life;y++){
    ...
    var ecost=energyCost*Math.pow(1.03,y-1), om=totalOM*Math.pow(1.02,y-1);
    var net=grid+host-ecost-om;
  ...
  return {
    ...
    grossCapex:grossCapex, itcAmt:itcAmt, itcBlendedPct:itcBlendedPct, storageRebate:storageRebate, netCapex:netCapex, depValue:depValue, monetizeDepr:monetizeDepr,
...
var CLAREMONT_PRESET = {
  ...
  iv_dep_on:true, iv_dep_monetize:true, iv_dep_tax:'21', iv_dep_stepup:true, iv_dep_steppct:'10',
...
function ivRenderReport(m){
  ...
  h+='<div class="card"><div class="card-body"><table class="pnl">'+
     ...
     (m.monetizeDepr&&m.depValue>0?pnl('Depreciation value (monetized)','NPV of MACRS tax shield', -m.depValue,'neg'):'')+
     '<tr class="grand"><td class="lbl">Net capital required</td><td class="num">'+fmt$(m.netCapex)+'</td></tr>'+
